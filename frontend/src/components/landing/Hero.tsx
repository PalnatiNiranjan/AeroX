...
export const Hero: React.FC = () => {
  ...
  // -----------------------------------------------------------------
  // Frame resolution + canvas draw — 0-50% -> aircraft, 50-100% -> nfc,
  // hard cut at the matched boundary frame. The nfc side reserves its
  // final slice of progress as a pure hold: playback completes at
  // (1 - NFC_HOLD_ZONE) and idx clamps to the last frame from there to
  // 1.0, so the canvas freezes well before the section unpins.
  //
  // REMOVED: a previous pass here added a runtime digital zoom/crop on
  // the last ~24 engine frames (extraScale ramping up to 2.4x,
  // re-centered on the chip) to hide the wiring cluster on the right.
  // That's gone — the wiring cluster is now removed at the pixel level
  // in the actual frame-090..120 JPEGs, so no runtime crop is needed,
  // and the crop was producing an unwanted progressive zoom-in after
  // "✓ Verified" landed. Draw is back to plain object-fit: cover, no
  // extra scale, no re-centering, for every frame in both sequences.
  // -----------------------------------------------------------------
  ...
  const drawFrame = useCallback((img: HTMLImageElement) => {
    ...
    // object-fit: cover, centered, no distortion. No extra zoom.
  ...
  return (
    <section
      ref={wrapperRef}
      className="relative w-full bg-[#e6edf1]"
